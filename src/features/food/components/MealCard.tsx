  ...
  return (
    <div className="flex items-center bg-card-app p-3 rounded-xl mb-2 gap-3">
      ...
      <div className="flex-1">
        ...
        <div className="text-xs text-text-light mt-0.5">          {formatTime(meal.timestamp)} · {meal.items.length} item
          {meal.items.length === 1 ? '' : 's'}
        </div>
      </div>
      <div className="flex flex-col items-end gap-1.5">
        <span className="text-sm font-bold text-primary">          {meal.meal_total.calories} kcal
        </span>
        <button
          type="button"
