...
function mockLogMeal(data: LogMealRequest): LogMealResponse {
  ...
  const items: FoodItem[] = data.items.map((it) => {
  ...
  const meal_total = items.reduce<NutritionTotals>(
    (acc, it) => ({
...
function mockSearch(query: string): SearchFoodResponse {
  const base: SearchFoodResult[] = [
    {
      fdc_id: 1001,
      name: `${query} (fresh)`,
      category: 'vegetable',
